...
from eigenstrapping_analysis.utils import (
    get_rootdir, get_medial, join, save_npy
)
...
medial_mask_10k = get_medial(density='10k')

# resample and save to arrays
HCP_thickness = fslr_to_fsaverage(
    data=join(OUTDIR, 'HCP_structural', 'space-fsLR_den-32k_hemi-lh_thickness.shape.gii'),
    target_density='10k',
    hemi='L'
)
HCP_thickness = HCP_thickness.agg_data()
HCP_thickness[np.logical_not(medial_mask_10k)] = np.nan
save_npy(join(OUTDIR, 'HCP_structural', 'HCP_thickness.npy'), HCP_thickness)

HCP_myelin = fslr_to_fsaverage(
    data=join(OUTDIR, 'HCP_structural', 'space-fsLR_den-32k_hemi-lh_myelin_average.func.gii'),
    target_density='10k',
    hemi='L'
)
HCP_myelin = HCP_myelin.agg_data()
HCP_myelin[np.logical_not(medial_mask_10k)] = np.nan
save_npy(join(OUTDIR, 'HCP_structural', 'HCP_myelin.npy'), HCP_myelin)

neurosynth = mni152_to_fsaverage(
    img=join(OUTDIR, 'neurosynth', 'space-MNI152_den-2mm_hemi-lh_neurosynth-gradient01.nii.gz'),
    fsavg_density='10k'
)[0] # we want L hemi only
neurosynth = neurosynth.agg_data()
neurosynth[np.logical_not(medial_mask_10k)] = np.nan
save_npy(join(OUTDIR, 'neurosynth', 'neurosynth_gradient01.npy'), neurosynth)
...
margulies_gradient[np.logical_not(medial_mask_10k)] = np.nan
save_npy(join(OUTDIR, 'margulies2016_PC1', 'margulies_gradient01.npy'), margulies_gradient)

genepc1 = nib.load(op.join(OUTDIR, 'abagen_PC1', 'space-fsaverage_den-10k_hemi-lh_abagen-PC1.shape.gii')).agg_data()
save_npy(join(OUTDIR, 'abagen_PC1', 'abagen_PC1.npy'), genepc1)

# convert and save 10k and 32k distance matrices
distmat_10k = join(OUTDIR, 'distmat', 'space-fsaverage_den-10k_hemi-lh_distmat.txt')
_ = txt2memmap(
    distmat_10k,
    output_dir=join(OUTDIR, 'distmat'),
    suffix='_10k'
)

distmat_32k = join(OUTDIR, 'distmat', 'space-fsLR_den-32k_hemi-lh_distmat.txt')
_ = txt2memmap(
    distmat_32k,
    output_dir=join(OUTDIR, 'distmat'),
    suffix='_32k'
